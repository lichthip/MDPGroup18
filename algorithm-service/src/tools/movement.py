"""Movement definitions for the robot."""
...
class Motion(int, Enum):
    ...
    def opposite_motion(self) -> "Motion":
        """Get the opposite motion (eg FORWARD -> REVERSE)."""
    ...
    def is_combinable(self) -> bool:
        """Check if this motion can be combined with others (only straight motions)."""
        if self == Motion.CAPTURE:
            return False
        return self in [Motion.REVERSE, Motion.FORWARD]

    def reverse_cost(self) -> int:
        """Get the cost penalty for reverse motions."""
        if self == Motion.CAPTURE:
